...
main(Attempts, Res) :-
	...
	asserta(sumOfDaysTeachersCanWork(X_can, Y_can, Z_can)),
	asserta(sumOfDaysTeachersWantWork(X_want, Y_want, Z_want)),
	initiateDijkstraSearch(Attempts, Attempts, 1000000, [], Res), !.
...
initiateDijkstraSearch(0, Attempts, Min, BestState, Res) :-
	((Min == 1000000, write('Не удалось составить расписание. Попыток было сделано: '), Res = [], writeln(Attempts), !);
	(not(Min == 1000000), writeln(''),
	TwiceSortedState = BestState,
	writeln('Лучшее расписание: '),
	Res = TwiceSortedState,
	printResult(TwiceSortedState),
	write('Попыток было сделано: '), writeln(Attempts),
	writeln('Сумма штрафов: '),
	writeln(Min), !)).
	
initiateDijkstraSearch(I, Attempts, Min, BestState, Res) :-
	clear,
	...
	New_I is I - 1,
	initiateDijkstraSearch(New_I, Attempts, New_Min, New_BestState, Res).
...
dijkstraSearch(node(State, Length, H), VacantClasses, X, Y) :-
	...
	%
	take_min(ListOfNeighbors, OptimalNeighbor),
...
take_min(List, Optimal_neighbor) :- take_min2(List, List, 0, 0, 100000, Optimal_neighbor).

take_min2([], List, _, MinInd, _, Optimal_neighbor) :- nth0(MinInd, List, X), Optimal_neighbor = X.
take_min2([node(_, _, SumH) | Other], List, Ind, MinInd, MinVal, Optimal_neighbor) :-
	((SumH < MinVal, NewMinInd is Ind, NewMinVal is SumH);
	(SumH >= MinVal, NewMinInd is MinInd, NewMinVal is MinVal)),
	NewInd is Ind + 1,!,
	take_min2(Other, List, NewInd, NewMinInd, NewMinVal, Optimal_neighbor).
...
classroom('т3213', [type_of_class('pr'), type_of_class('lab')], 20).
classroom('402 ГК', [type_of_class('lec'), type_of_class('pr')], 200).
classroom('СК', [type_of_class('pe')], 500).
classroom('т2266', [type_of_class('pr'), type_of_class('lab')], 20).
classroom('2128', [type_of_class('lec'), type_of_class('pr')], 80).
classroom('т3220', [type_of_class('pr'), type_of_class('lab')], 20).
classroom('1154', [type_of_class('lec'), type_of_class('pr')], 20).
classroom('406 ГК', [type_of_class('lec'), type_of_class('pr')], 20).
classroom('1155', [type_of_class('lec'), type_of_class('pr')], 40).
classroom('т2213', [type_of_class('pr'), type_of_class('lab')], 20).
%classroom('т2221', [type_of_class('pr'), type_of_class('lab')], 20).
...
attempts(1)semester(1)study_days_in_week(6).
study_days_in_week_students(6).
study_days_in_week_teachers(5).
classes_in_day(7).
classes_in_day_students(2).
classes_in_day_teachers(1).
type_of_class('pr').type_of_class('lec').type_of_class('lab').type_of_class('misc').type_of_classroom('huge for lectures and practices', [type_of_class('lec'), type_of_class('pr')], 200).type_of_classroom('big for lectures and practices', [type_of_class('lec'), type_of_class('pr')], 80).type_of_classroom('medium for lectures and practices', [type_of_class('lec'), type_of_class('pr')], 40).type_of_classroom('small for lectures and practices', [type_of_class('lec'), type_of_class('pr')], 20).type_of_classroom('terminals', [type_of_class('pr'), type_of_class('lab')], 20).type_of_classroom('room for pe', [type_of_class('pe')], 500).faculty('fit').
ed_program('fit', 'Новый поток').
ed_program('fit', 'cewc').
specialization('Новый поток', 'Компуктер сайнс').
specialization('cewc', ' fdcre').
