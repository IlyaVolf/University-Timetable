...
take_min2([node(_, _, SumH) | Other], List, Ind, MinInd, MinVal, Optimal_neighbor) :-
	((SumH =< MinVal, NewMinInd is Ind, NewMinVal is SumH, NewInd is Ind + 1);
	(SumH > MinVal, NewMinInd is MinInd, NewMinVal is MinVal, NewInd is Ind + 1)),
...
% classroom(building('Новый корпус'), '1156', Types_of_class, Capacity_of_classroom) :- type_of_classroom('huge for lectures and practices', Types_of_class, Capacity_of_classroom).
