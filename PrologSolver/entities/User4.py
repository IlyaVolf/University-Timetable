...
#       return dbManager.getUser(int(user_id))
...
#     cursor.close()
#
#     user = User(str(row[0]), row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9])
#
...
class User(UserMixin):
    ...
    def updateUser(self, name, email, role, teacherId=None):
        ...
        date = datetime.datetime.now()
        cursor.execute(sqliteQuery, (name, email, role, teacherId, date, int(self.id)))
    ...
    def signUpUser(self, password):
        ...
        date = datetime.datetime.now()
        cursor.execute(sqliteQuery, (passwordHash, date, date, int(self.id)))
