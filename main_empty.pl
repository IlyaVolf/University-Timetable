...
main(Attempts, Res, Fine) :-
	clear_all,
	sumOfAllDaysTeachersCanWork(X_can, Y_can, Z_can),
	sumOfAllDaysTeachersWantWork(X_want, Y_want, Z_want),
	asserta(sumOfDaysTeachersCanWork(X_can, Y_can, Z_can)),
	asserta(sumOfDaysTeachersWantWork(X_want, Y_want, Z_want)),
	initiateDijkstraSearch(Attempts, Attempts, 1000000, [], Res, Fine), !.
...
initiateDijkstraSearch(0, Attempts, Min, BestState, Res, Fine) :-
	%((Min == 1000000, write('Не удалось составить расписание. Попыток было сделано: '), Res = [], writeln(Attempts), !);
	((Min == 1000000, write('FAIL'), Res = [], Fine = -1, writeln(Attempts), !);
...
initiateDijkstraSearch(I, Attempts, Min, BestState, Res, Fine) :-
	clear,
	...
	New_I is I - 1,
	initiateDijkstraSearch(New_I, Attempts, New_Min, New_BestState, Res, Fine).
